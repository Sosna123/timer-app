<template>
    <div class="timerDiv pa-5 bg-tertiary d-flex" style="justify-content: center; align-items: center">
        <div style="" class="d-flex timerContainer" @touchend="screenWidth <= 600 ? manageTimer() : null">
            <div v-if="timerMode == 'normal'" id="timerOverflowDiv">
                <p
                    :class="{
                        'text-amber': spaceDown,
                        'text-black': $props.currTheme === 'light',
                        'text-amber-lighten-1': spaceDown && $props.currTheme === 'amber',
                    }"
                    id="timerText"
            ...
            <div v-else-if="timerMode == 'input'" style="width: 60%">
                <v-text-field class="timeInput" hide-details="auto" label="Input your time" v-model="timeInInput"></v-text-field>
                <v-btn style="width: 100%" @click="submitInputTime()" color="background"><button>Submit</button></v-btn>
            </div>
        </div>
        <v-btn
            style="position: absolute; bottom: 0; right: 0"
            @click="
                timerMode == 'input' ? (timerMode = 'normal') : (timerMode = 'input');
...
const props = defineProps(["removeTime", "currTheme", "editingOptions", "showTimeList", "timeList", "currentScramble"]);
type Time = {
    id: number;
    str: string;
    num: number;
...
window.document.addEventListener("load", () => {
    changeModeBtn = document.getElementById("changeMode") as HTMLButtonElement;
});
...
function manageTimer() {
    ...
    if (!(!props.editingOptions && timerMode.value == "normal" && !props.showTimeList)) return;
...
function changeOverflowFontSize() {
    if (timerMode.value == "normal") {
        ...
        const timerOverflowDiv = document.getElementById("timerOverflowDiv")!;
